...
public:
  ...
  friend class TrackingRecHit2DHeterogeneous;
   ...
   inline  float_st& xLocal(int i) { return m_xl[i]; }
   inline  float_st xLocal(int i) const { return m_xl[i]; }
   inline  float_st& yLocal(int i) { return m_yl[i]; }
   inline  float_st yLocal(int i) const { return m_yl[i]; }

   inline  float_st& xerrLocal(int i) { return m_xerr[i]; }
   inline  float_st xerrLocal(int i) const { return m_xerr[i]; }
   inline  float_st& yerrLocal(int i) { return m_yerr[i]; }
   inline  float_st yerrLocal(int i) const { return m_yerr[i]; }

   inline  float_st& xGlobal(int i) { return m_xg[i]; }
   inline  float_st xGlobal(int i) const { return m_xg[i]; }
   inline  float_st& yGlobal(int i) { return m_yg[i]; }
   inline  float_st yGlobal(int i) const { return m_yg[i]; }
   inline  float_st& zGlobal(int i) { return m_zg[i]; }
   inline  float_st zGlobal(int i) const { return m_zg[i]; }
   inline  float_st& rGlobal(int i) { return m_rg[i]; }
   inline  float_st rGlobal(int i) const { return m_rg[i]; }
...
private:
  // local coord
  float_st *m_xl, *m_yl;
  float_st *m_xerr, *m_yerr;

  // global coord
  float_st *m_xg, *m_yg, *m_zg, *m_rg;
  int16_t* m_iphi;
