...
void kernel_print_found_ntuplets(TrackingRecHit2DSOAView const *__restrict__ hhp,
  ...
  auto const &tracks = *ptracks;
  int first = 0;
  for (int i = first, np = fmin(maxPrint, foundNtuplets.nbins()); i < np; i += 1) {
    ...
    printf("TK: %d %d %d %f %f %f %f %f %f %f %d %d %d %d %d\n",
           ...
           nh,
           tracks.charge(i),
           static_cast<float>(tracks.pt(i)),
           static_cast<float>(tracks.eta(i)),
           tracks.phi(i),
           tracks.tip(i),
           tracks.zip(i),
           //           asinhf(fit_results[i].par(3)),
           static_cast<float>(tracks.chi2(i)),
