...
#endif
    constexpr u_int n = N;
    double p_t = fmin(20., fast_fit(2) * B);  // limit pt to avoid too small error!!!
...
*/
  ...
  inline circle_fit Circle_fit(const M2xN& hits2D,
    ...
    const double_st h = sqrt(1. - sqr(v(2)) - 4. * c * v(2));
    const double_st v2x2_inv = 1. / (2. * v(2));
    const double_st s_inv = 1. / s;
    ...
    if (error) {
      ...
      Eigen::Matrix<double_st, 3, 6> J2;  // Jacobian of min_eigen() (numerically computed)
      for (u_int a = 0; a < 6; ++a) {
        const u_int i = nu[a][0], j = nu[a][1];
 ...
 */
  ...
  inline line_fit Line_fit(const M3xN& hits,
    ...
    double_st m = common_factor * (sol(1, 0) * sin(theta) + cos(theta));
    double_st q = common_factor * sol(0, 0);
    auto cov_mq = J * Cov_params * J.transpose();

    VectorNd<N> res = p2D_rot.row(1).transpose() - A.transpose() * sol;
    double_st chi2 = res.transpose() * Vy_inv * res;
