...
void HelixFitOnGPU::launchRiemannKernelsOnCPU(HitsView const *hv, uint32_t nhits, uint32_t maxNumberOfTuples) {
  ...
  //  Fit internals
  auto hitsGPU_ = std::make_unique<double_st[]>(maxNumberOfConcurrentFits_ * sizeof(Rfit::Matrix3xNd<4>) / sizeof(double_st));
  auto hits_geGPU_ = std::make_unique<float_st[]>(maxNumberOfConcurrentFits_ * sizeof(Rfit::Matrix6x4f) / sizeof(float_st));
  auto fast_fit_resultsGPU_ =
      std::make_unique<double_st[]>(maxNumberOfConcurrentFits_ * sizeof(Rfit::Vector4d) / sizeof(double_st));
