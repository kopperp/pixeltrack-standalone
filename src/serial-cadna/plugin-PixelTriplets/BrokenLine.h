...
namespace BrokenLine {
  ...
  */
  inline double_st MultScatt(const double_st& length, const double_st B, const double_st R, int Layer, double_st slope) {
    // limit R to 20GeV...
    auto pt2 = fmin(20., B * R);
    pt2 *= pt2;
  ...
  */
  inline Rfit::Matrix2d RotationMatrix(double_st slope) {
    Rfit::Matrix2d Rot;
  ...
  */
  inline void TranslateKarimaki(karimaki_circle_fit& circle, double_st x0, double_st y0, Rfit::Matrix3d& jacobian) {
    double_st A, U, BB, C, DO, DP, uu, xi, v, mu, lambda, zeta;
  ...
  */
  ...
  inline void prepareBrokenLineData(const M3xN& hits,
    ...
    const double_st slope = -results.q / fast_fit(3);
...
template <typename M3xN, typename M6xN, typename V4, int N>
  inline void BL_Circle_fit(const M3xN& hits,
    ...
    const double_st slope = -circle_results.q / fast_fit(3);
    ...
    double_st tmp1 = eMinusd.squaredNorm();
  ...
  */
  ...
  inline void BL_Line_fit(const M6xN& hits_ge,
    ...
    const double_st slope = -data.q / fast_fit(3);
    Rfit::Matrix2d R = RotationMatrix(slope);
