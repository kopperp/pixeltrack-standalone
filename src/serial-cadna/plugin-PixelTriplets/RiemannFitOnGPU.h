...
void kernelLineFit(CAConstants::TupleMultiplicity const *__restrict__ tupleMultiplicity,
...
#if CADNA_DEBUG
    const int in_digits = phits->nb_significant_digit();
#endif
  ...
  for (int local_idx = local_start, nt = Rfit::maxNumberOfConcurrentFits(); local_idx < nt; local_idx++) {
    ...
    results->stateAtBS.copyFromCircle(
        circle_fit[local_idx].par, circle_fit[local_idx].cov, line_fit.par, line_fit.cov, 1.f / float_st(B), tkid);
...
#if CADNA_DEBUG
    std::cout << "-- RIEMANN FIT --" << std::endl;
    print_cadna_metric("Line pt", results->pt(tkid), in_digits);
    print_cadna_metric("Line eta", results->eta(tkid), in_digits);
    print_cadna_metric("Line Chi2", results->chi2(tkid), in_digits);
#endif
