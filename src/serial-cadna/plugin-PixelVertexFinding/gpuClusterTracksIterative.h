...
namespace gpuVertexFinder {
  ...
  void clusterTracksIterative(ZVertices* pdata,
    ...
    if (verbose)
      printf("params %d %f %f %f\n", minT, static_cast<double>(eps), static_cast<double>(errmax), static_cast<double>(chi2max));
    ...
    for (uint32_t i = 0; i < nt; i++) {
      ...
      iz = fmin(std::max(iz, INT8_MIN), INT8_MAX);
      izt[i] = iz - INT8_MIN;
    ...
    while (more) {
      if (1 == nloops % 2) {
        ...
        for (uint32_t k = 0; k < hist.size(); k++) {
          auto p = hist.begin() + k;
          auto i = (*p);
          auto be = fmin(Hist::bin(izt[i]) + 1, int(hist.nbins() - 1));
          if (nn[i] < minT)
