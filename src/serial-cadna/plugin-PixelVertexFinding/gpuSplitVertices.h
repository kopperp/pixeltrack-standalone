...
namespace gpuVertexFinder {
  ...
  inline void splitVertices(ZVertices* pdata, WorkSpace* pws, float maxChi2) {
    ...
    auto nt = ws.ntrks;
    float_st const* __restrict__ zt = ws.zt;
    float_st const* __restrict__ ezt2 = ws.ezt2;
    float_st* __restrict__ zv = data.zv;
    float_st* __restrict__ wv = data.wv;
    float_st const* __restrict__ chi2 = data.chi2;
    ...
    for (uint32_t kv = 0; kv < nvFinal; kv += 1) {
      ...
      float_st zz[MAXTK];   // z pos
      uint8_t newV[MAXTK];  // 0 or 1
      float_st ww[MAXTK];   // z weight
      ...
      float_st znew[2], wnew[2];  // the new vertices
      ...
      if (verbose)
        printf("inter %d %f %f\n", 20 - maxiter, static_cast<double>(chi2Dist), static_cast<double>(dist2 * wv[kv]));
  ...
  void splitVerticesKernel(ZVertices* pdata, WorkSpace* pws, float_st maxChi2) { splitVertices(pdata, pws, maxChi2); }
