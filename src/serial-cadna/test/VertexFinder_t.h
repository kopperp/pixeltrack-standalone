...
struct Event {
  std::vector<float_st> zvert;
  std::vector<uint16_t> itrack;
  std::vector<float_st> ztrack;
  std::vector<float_st> eztrack;
  std::vector<float_st> pttrack;
  std::vector<uint16_t> ivert;
};

struct ClusterGenerator {
  explicit ClusterGenerator(float_st nvert, float_st ntrack)
...
int main() {
  ...
  float_st eps = 0.1f;
  std::array<float_st, 3> par{{eps, 0.01f, 9.0f}};
  for (int nav = 30; nav < 80; nav += 20) {
    ...
    for (int i = 8; i < 20; ++i) {
      ...
      ::memcpy(LOC_WS(zt), ev.ztrack.data(), sizeof(float_st) * ev.ztrack.size());
      ::memcpy(LOC_WS(ezt2), ev.eztrack.data(), sizeof(float_st) * ev.eztrack.size());
      ::memcpy(LOC_WS(ptt2), ev.pttrack.data(), sizeof(float_st) * ev.eztrack.size());
      ...
      float_st* zv = nullptr;
      float_st* wv = nullptr;
      float_st* ptv2 = nullptr;
      int32_t* nn = nullptr;
      uint16_t* ind = nullptr;

      // keep chi2 separated...
      float_st chi2[2 * nv];  // make space for splitting...
      ...
      memcpy(chi2, LOC_ONGPU(chi2), nv * sizeof(float_st));
...

      {
        ...
        std::cout << "min max error " << 1. / sqrt(*mx.first) << ' ' << 1. / sqrt(*mx.second) << std::endl;
      }
